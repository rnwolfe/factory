...
- **Wide `bun test` is safe again (the parent-kill bug is fixed).** Previously, wide-scope invocations (`bun test apps/daemon/`, `bun --filter '@factory/daemon' test`, the full-suite gate `bun run test`) reproducibly killed the parent `claude --print` process within ~5s — no `factory-status` footer, run marked `failed` despite committed work. **Root cause:** Factory's daemon tmux integration tests (`sessions-orchestrate` / `interventions-orchestrate`) created and killed *real* tmux sessions on the **shared default tmux server**; when the suite ran inside a self-hosting run, that churn destabilized the server the parent pane lived on. (The old "smoking gun" guess — a session-name collision — was wrong; it's server/socket-level.) **Fix:** those suites now set `FACTORY_TMUX_SOCKET`, so every tmux command Factory issues passes `-L <socket>` (`tmuxSocketArgs` in `packages/runtime/src/tmux.ts`) and lands on a throwaway server, isolated from any ambient one. It must be a CLI flag, not an env tweak — Bun snapshots a child's env at spawn, so unsetting `$TMUX`/`TMUX_TMPDIR` in-process does **not** reach the spawned tmux. Verified: the full daemon suite (363 tests) runs to completion with a co-tenant parent pane surviving, with `$TMUX` faked to the parent socket. `FACTORY_TMUX_SOCKET` is unset in production → no flag → tmux behavior is byte-identical. If a wide run ever kills a run again, re-check whether a new tmux call site bypassed `tmuxSocketArgs()`.
...
- Project bootstrap: `apps/daemon/src/projects/bootstrap.ts`. Task IO: `tasks.ts` (single point of truth for task creation; `createTask` consumed by bootstrap, refinement-freeze, feature-plan-freeze, finding-promote-bug). **The `createTask` seam normalizes every task identically — no caller can diverge:** it guarantees an `## Acceptance` section (`ensureAcceptanceSection`, so the verifier's acceptance signal is never permanently absent) and cleans the `blockedBy` dependency edges (`normalizeBlockedBy`). Task **dependencies** (ADR-019): a task is *startable* only when every `blockedBy` dep is `done`/`dropped` (`isStartable`); `pickNextReadyTask` enforces this so auto-advance respects ordering. Edges live in the file backend's frontmatter and the GitHub backend's meta block; `setBlockedBy` rejects cycles. Derived state (`startable`/`openBlockers`/`blocks`) is never persisted.
